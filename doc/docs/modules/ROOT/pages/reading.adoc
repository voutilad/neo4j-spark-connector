...
Since partitioning is done by using LIMIT/SKIP you should not limit the result yourself.

To achieve this you can use `query.count` option (more on it <<quickstart.adoc#bookmark-parallelize,here>>)
by specifying the number of records you want to limit your dataset.

Please don't use SKIP/LIMIT at the end of your query.
Using it _in the middle_ of the query is still ok.
...
spark.read.format("org.neo4j.spark.DataSource")
  .option("url", "bolt://localhost:7687")
  .option("query", "MATCH (n:MY_ONE_THOUSAND_NODES_LABEL) RETURN n")
  .option("query.count", "100")
  .load()
----

If you limit your query you will get an error since the limit is already added by the partitioning system.
